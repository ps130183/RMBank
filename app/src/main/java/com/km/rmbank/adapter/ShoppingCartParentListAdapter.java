...
public class ShoppingCartParentListAdapter extends BaseAdapter<ShoppingCartEntity> implements BaseAdapter.IAdapter<ShoppingCartParentListAdapter.ViewHolder>,BaseAdapter.ItemClickListener<GoodsDto> {
    ...
    public void createView(final ViewHolder holder, final int position) {
        final ShoppingCartEntity cartEntity = getItemData(position);
        holder.subListAdapter.addData(cartEntity.getGoodsEntities());
//        holder.subListAdapter.setParentListAdapter(this,position);
        ...
        holder.mCheckBox.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                ...
                //修改子级选择状态
                checkAllSubByParent(cartEntity.getGoodsEntities(),isChecked,holder);
                //检测是否全选
                checkAll();
            }
        });
     ...
     * @param holder
     */
    private void checkAllSubByParent(List<GoodsDto> goodsEntities, boolean isChecked, ViewHolder holder){
//        Logger.d("checkAllSubByParent isParentCheckChange = " + isParentCheckChange );
        if (isParentCheckChange){
            for (GoodsDto entity : goodsEntities){
    ...
    public void checkParentBySub(final int position){
        ShoppingCartEntity cartEntity = getItemData(position);
        cartEntity.setChecked(isSubAllChecked(cartEntity.getGoodsEntities()));
        isParentCheckChange = false;
        checkAll();
     ...
     * @return
     */
    public boolean isSubAllChecked(List<GoodsDto> goodsEntities){
    ...
    private boolean isParentAllChecked(){
        boolean isChecked = true;
        for (ShoppingCartEntity entity : getAllData()){
            if (!entity.isChecked()){
                isChecked = false;
                break;
            }
...
//            Logger.d("isParentAllChecked = " + isParentAllChecked());
    ...
    public void checkAll(boolean isCheckAll){
        for (ShoppingCartEntity entity : getAllData()){
            ...
            for (GoodsDto entity1 : entity.getGoodsEntities()){
    ...
    public interface OnSubItemClcikListener{
        void onSubItemClick(GoodsDto itemData, int position);
    }
